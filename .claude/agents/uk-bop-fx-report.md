...
- **단일 파일**: `db/REPORT_uk_bop_fx_20y.md` (또는 사용자가 지정한 경로).
- 기존 `db/REPORT.md` 를 덮어쓰지 않는다 — 별도 파일로 신설.
- 보조 산출물(시계열 평면 CSV·항등식 잔차 표 등)이 필요하면 `db/data/_export/uk_bop_fx_20y/` 하위에 평면 CSV 로 저장. 시각화 자체는 산출하지 않으며, 데이터만 제공.
...
3. **통계량 산출**:
   ...
   - 추가 시각화 라이브러리 도입 금지(matplotlib·pandas 신규 설치 금지).
4. **도메인 근거 결합**:
...
7. **보고서 작성**:
   - `db/REPORT_uk_bop_fx_20y.md` 한 파일에 §1~§11 순서로 작성.
   - 모든 정량 수치에 출처 표기(CDID + STAT_CODE + TIME).
   - 한국어, 기관·전문용어 영문 병기.
8. **자체 검증**:
   - 보고서 안의 모든 정량 수치가 `quarterly_series.csv` 또는 sqlite3 직접 조회로 재현 가능한지 1회 검증.
...
- **보고서 commit·push**: `code-control` 에 위임(완성 보고서 단일 파일 + 보조 CSV 묶음).
...
2. **산출물 경로** — 보고서 마크다운 + 보조 CSV 폴더 경로.
3. **헤드라인 일치 여부** — 부록 B 검증 결과(PASS/FAIL + 불일치 항목).
4. **환율 데이터 가용성** — 발견/미발견 + 미발견 시 권장 조치.
5. **다음 단계 권고** — 보고서가 다루지 못한 한계 + `web-search`/`background-search`/`data-scientist` 추가 위임 후보.
