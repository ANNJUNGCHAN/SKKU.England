...
|---|---:|---|
| `x` | 360 | ONS 비공개(suppressed): 응답 기관 식별 위험. GAF 권고 기호 `[c]`로 단일 통합 가능 |
...
- **`x` → 원문 보존**: `value_raw="x"`, `value_numeric=NULL`, `missing_reason="confidential"` 3-컬럼 패턴(15회차 권고 재확인). 0이나 NaN으로 임의 치환 금지(db/CLAUDE.md "값 불변" 원칙).
- **trailing padding `(empty)` → 적재 대상 외**: 데이터 영역(CDID 컬럼 × 첫 분기 이후 행) 외부 빈 셀은 ECOS 관측치 테이블에 행으로 적재하지 않음.
- **Table_C 1997·1998 사용 주의**: 사용자 인터페이스 안내 문구에 EU 시계열은 1999 Q1부터 사용 가능함을 명시.
