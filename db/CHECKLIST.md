...
- [x] **[web-search]** 中 — BoE Bankstats(Tables A–C) 또는 BoE Statistical Release에서 영국 보유 준비자산(IR) 시계열 확보. 슬라이드 24의 NFA(중앙은행)와 슬라이드 11의 Net IIP(국가 전체) 정량 분리 — 산출물: `background/note/17_boe_reserve_assets.md` + `17_boe_reserve_assets.csv` (출처 20건). 핵심 발견: **EEA(IR 법적 보유 주체) ≠ BoE 대차대조표** — 영국 IR은 HMT 소유·BoE 운영. 2025-10末 $112,358m / £85,518m vs 같은 시점 Net IIP −£200bn (부호·규모 모두 다름). BoE Bankstats 시리즈 ID `ACLRA`·`ATOTRA` 확인. 미해결 4건(Bankstats 표 코드 / IR 구성요소 분해 / "$180bn" 메모 출처 / BoE 자체 NFA mnemonic) 노트 §6 기록
- [x] **[background-search]** 中 — ONS Pink Book Chapter 9 (IIP) xlsx에서 영국의 재평가(가격·환율·기타) 3분해 시계열 추출 → 슬라이드 26 매트릭스 비거래요인 정량 검증 — 산출물: `background/note/18_iip_revaluation_decomp.md` + `18_iip_revaluation_decomp.csv` (출처 17건). 핵심 발견: **분해 시계열은 분기 BoP 자료에 없고, 연간 Pink Book bulletin Figure 12·13(자산·부채 각각 4분해 Flows/Currency/Price/Other × 2008–2024) CSV로 1차 공개됨**. chapter8.xlsx(13시트)는 추가 inventory 후 재평가 분해 시트 존재 여부 확정 필요. ONS 2020 article은 sector × 4분해 Q1 2000–Q4 2019 한정 historical 출처. **분기점**: Figure 12·13 CSV + chapter8.xlsx를 `db/source/`에 신규 추가할지 사용자 결정 대기 (옵션 A 부분추가 권고 / B 노트만 / C IMF SDMX 확장)
